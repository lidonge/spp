1. # S++Reference Logical Architecture
   ...
   - The quantum service layer corresponds to the smallest granularity of services, equivalent to traditional microservices
   - The scenario service layer corresponds to the combination of minimum granularity, which is equivalent to the service orchestration under the traditional micro-service architecture, and corresponds to the business service under the traditional SOA architecture.
   ...
   The entire S++ recommendation is deployed through the microservices container cloud, making it easy to use the existing tools of the microservices architecture, as well as the rapid deployment and dynamic scaling capabilities of the container cloud.

   ## quantum service layer

   The quantum service layer corresponds to the microservice layer of the microservice architecture. The quantum service layer has the smallest granularity and can be as small as a single attribute that manipulates participants. The services of the quantum service layer will only access the data layer and will not access other services, so it can be implemented in a very simple framework. The basic content of the framework of the quantum service layer only needs to include the following:
   ...
   - Required management services (service registration tool, IP whitelist management)
   - Security component based on dynamic IP whitelist
   - Service code automatically generated based on the business model (including the correct code)
   - Service polymorphism implementation

   The simpler the quantum service framework, the more friendly the dynamic scalability of the system as a whole. When the start and stop time of the quantum service container is shortened to the order of tens of milliseconds, the response capability of the service service to the sudden change of the load is basically close to the response capability of the preset redundant node under the cluster. Therefore, from the perspective of dynamic scaling, the granularity of the service is indeed as small as possible.

   <img src="./quantums.png" width="500" align=center />

   ## Scene Service Layer

   The service scenario layer corresponds to the traditional SOA service layer. If the business scenario combination is implemented by SOA, it is a traditional high-cohesion loosely coupled SOA service. The operation framework of the scene service layer should meet the following requirements:
   ...
   - Need to implement parallel combined trading engine
   - Need to provide the necessary adaptation technology services

   <img src="./gateway.png" width="800" align=center />

   The scene service gateway can be modified by using the existing micro-service gateway, and mainly adds a scenario combination service engine, an error handling interceptor, and a combination correction module. In the S++ architecture, since zero-coupling between services is implemented in the quantum service layer, zero-coupling between services is recommended in the overall architecture, that is, any two services (whether quantum services, SOA services, or computing services) It is not recommended to call each other directly, and all calls must be completed through the process.
   ...
   - Release: All services (except quantum services) and processes can be published. The service publisher sends the results of the service itself to the queue specified by the platform. The process publisher publishes the data and context of the next compute node to Specify the queue.
   - Subscriptions: All services (except quantum services) and processes can be subscribed, but only processes can subscribe to services, and services cannot subscribe directly to services (subscription processes only).
   - Other event sources: Other event sources can only participate as services, typically timed events and external events.
   - Queue message persistence: It is recommended to only persist the release queue corresponding to the manual process, because the human interface process and the automated process are time-sensitive.
   ...
   - Business services can be rushed: Business services, including composite services and SOA services, must provide the appropriate rush capabilities.
   ...
   2. The publish subscription service listens to the SDO, finds the in-service subscription table, finds a list of all processes that focus on the service S, and returns to the process F.
   3. The publish subscription service invokes the gateway router and accesses process F with SDO.
   4. The router obtains the protocol of the subscriber queue according to the address of the process F registered by the registration center.
   ...
   3. The SOA query service or the combined query service receives the query event request, executes the query task, and invokes the gateway to generate the query result event.
